...
import { createNodeRecord, getNodeMeta } from "../config/nodeTypes";

const FlowContext = createContext(null);

const initialState = {
  nodes: [
    createNodeRecord({ id: "node_1", x: 250, y: 72, type: "initial", label: "Initial Node" }),
    createNodeRecord({ id: "node_2", x: 250, y: 238, type: "branch", label: "Branch Node" }),
    createNodeRecord({ id: "node_3", x: 24, y: 406, type: "transform", label: "Transform Node" }),
    createNodeRecord({ id: "node_4", x: 454, y: 406, type: "output", label: "Output Node" }),
    createNodeRecord({ id: "node_5", x: 250, y: 572, type: "output", label: "Output Node" }),
  ],
  edges: [
    { id: "e1", source: "node_1", target: "node_2", label: "", animated: false },
    { id: "e2", source: "node_2", target: "node_3", label: "", animated: false },
    { id: "e3", source: "node_2", target: "node_4", label: "", animated: false },
    { id: "e4", source: "node_3", target: "node_5", label: "", animated: false },
  ],
  viewport: { x: 140, y: 40, zoom: 0.92 },
  selectedEdge: null,
  connecting: null,
...
function flowReducer(state, action) {
  switch (action.type) {
    ...
    case "ADD_NODE": {
      ...
      const nodeType = action.nodeType ?? "transform";
      return {
        ...state,
        nodes: [
      ...
      const nodes = state.nodes.map((n) =>
        n.id === action.id
          ? {
              ...
              data: {
                ...n.data,
                iconKey: action.nodeType,
                displayTitle: getNodeMeta(action.nodeType).label,
              },
            }
          : n
      );
    ...
    case "DROP_CONNECT_ON_PANE": {
      ...
      const newNode = createNodeRecord({
        id: newId,
        x: action.x,
        y: action.y,
        type: "transform",
    ...
    case "INSERT_NODE_ON_EDGE": {
      ...
      const newId = utils.id("node");
      const meta = getNodeMeta(action.nodeType);
      const mid = utils.getEdgeMidpoint(source, target);
      const x = mid.x - meta.defaultWidth / 2;
      const y = mid.y - meta.defaultHeight / 2;
      const newNode = createNodeRecord({
        id: newId,
        x,
        y,
    ...
    case "DUPLICATE_SELECTED": {
      ...
      return {
        ...state,
        nodes: [...state.nodes.map((n) => ({ ...n, selected: false })), ...duplicatedNodes],
    ...
    case "FIT_VIEW": {
      ...
      const zoom = utils.clamp(
        Math.min((action.w - pad * 2) / (maxX - minX), (action.h - pad * 2) / (maxY - minY)),
        utils.MIN_ZOOM,
        utils.MAX_ZOOM
      );
